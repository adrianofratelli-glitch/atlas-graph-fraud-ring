...
On the demo dataset, the group behind that company holds **R$ 154 million** in
limits across 43 companies over six levels of ownership, and **R$ 10.7 million
overdue** — none of it in the applicant's record. It sits in sibling companies in another branch, and no analyst
...
16 milliseconds over a base of 1.2 million companies and 2.5 million ownership
edges — and six levels of ownership cost 3 ms more than one. Then it answers a second one with the same mechanism: **which accounts may
this user see?**
...
three companies, R$ 3.7 million in limits, nothing overdue. This is what the credit
system shows today.


![Depth 1: the applicant and who controls it directly — three companies, R$ 3.7 M in limits, nothing overdue](docs/screenshots/01-empresa-sozinha.png)
### 2. Raise the depth

Each applicant in the picker belongs to a group with a **different** ownership
depth, from 1 to 6 levels — the dropdown says which. On the six-level group every
step of the control reveals companies the previous one did not: 3, then the chain
climbs, and at depth 6 it reaches the holding and opens to **43 companies**
including subsidiaries the applicant's record never mentions. Consolidated limits
go from R$ 3.7 million to **R$ 154.5 million**, and **R$ 10.7 million** of arrears
appear in a branch the applicant is not part of.

Ask for depth 6 on the one-level group and you get the same answer as depth 1, at
the same latency: the traversal ends when the tree ends.
...
![Depth 6: the group opens to 43 companies over six levels, R$ 154.5 M in limits and R$ 10.7 M overdue in a branch the applicant is not part of](docs/screenshots/02-grupo-economico.png)
...
That transaction is also what everyone else finds out from. A change stream on
`companies` picks up the marked documents, coalesces the forty-odd events into one
per case, and the alert lands on screen on its own — no polling, no scheduled job —
carrying the case, the company count and the exposure now under review. Closing the
review returns the counterpart event by the same path, because silence must be
provably correct rather than a broken listener.


![Review open over all 43 companies in one ACID transaction, and the change-stream event it produced: 43 companies under review, R$ 154.5 M of exposure, delivered in 515 ms](docs/screenshots/07-acid-e-alerta.png)
...
| User | Sees, of the group's 43 companies |
|---|---|
| advisor | 15 |
| their manager | **29** |
| the regional above | **43** |
...
![Visibility panel with a manager selected: 29 of the group's 43 companies in scope, 16 advisors, and the accounts outside it dimmed on the graph](docs/screenshots/05-visibilidade-gerente.png)

![The same account refused to an advisor in another branch — 15 of 43 in scope — with the reason and the advisor who owns it](docs/screenshots/06-visibilidade-assessor.png)
...
Over an 8.5 ms network floor, the economic-group query costs **5 to 8 ms** of actual
work, from a one-level group to a six-level one, and an advisor's book costs about
9 ms. A manager's book — 16 advisors, 6,300 accounts with credit — costs 35 ms. A
regional level, 129 advisors and 51,000 accounts, takes about 460 milliseconds, and
that is the honest ceiling of this shape.
